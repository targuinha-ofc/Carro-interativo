// script.js - Garagem Inteligente Pro (Versão Final, Limpa e Corrigida)

// ======================================================================================
// PASSO CRÍTICO: COLE A URL CORRETA DO SEU BACKEND PUBLICADO NO RENDER AQUI!
...
async function fetchAndDisplayMaintenanceTips(vehicleType) {
    ...
    tipsContentEl.innerHTML = `<p class="loading-message">Buscando dicas...</p>`;
    try {
        const response = await fetch(`${BACKEND_URL}/api/dicas/${vehicleType}`);
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.message || `Falha ao buscar dicas: ${response.statusText}`);
        }
        const dicas = await response.json();
        tipsContentEl.innerHTML = `<ul>${dicas.map(dica => `<li>${dica}</li>`).join('')}</ul>`;
    } catch (error) {
        console.error("Erro ao buscar dicas:", error);
        tipsContentEl.innerHTML = `<p class="error-message">${error.message}</p>`;
    }
}

async function fetchGenericArsenalData(endpoint, containerId, renderer, featureName) {
    const container = document.getElementById(containerId);
    container.innerHTML = `<p class="loading-message">Carregando ${featureName}...</p>`;
    try {
        ...
        if (!data || data.length === 0) {
            container.innerHTML = `<p class="info-message">Nenhum item encontrado para ${featureName}.</p>`;
            return;
        }
        container.innerHTML = renderer(data);
    } catch (error) {
        console.error(`Erro ao buscar ${featureName}:`, error);
        container.innerHTML = `<p class="error-message">Falha ao carregar ${featureName}. Verifique a URL do backend e se ele está online.</p>`;
    }
}
    ...
    fetchGenericArsenalData('veiculos-estrela', 'veiculos-estrela-container', data =>
        data.map(v => `
            <div class="arsenal-card">
                <img src="${v.imagemURL}" alt="Imagem de ${v.modelo}" loading="lazy" onerror="this.onerror=null; this.src='${FALLBACK_IMAGE_URL}';">
                <div class="arsenal-card-content">
                    <h4>${v.modelo}</h4>
                    <p>${v.descricao}</p>
                </div>
            </div>
        `).join(''), 'veículos estrela');
...
class Agendamento {
    ...
    get dataFormatada() { return new Date(this.dataAgendamento).toLocaleDateString('pt-BR', { timeZone: 'UTC' }); }
}
...
class Veiculo {
    constructor(modelo, cor, imagemURL = '') {
        ...
        this.ligado = false;
        this.kmRodados = 0;
        this.manutencao = { oleo: { proximaKm: OIL_CHANGE_INTERVAL, ultimaKm: 0, status: "OK" }, pneus: { proximaKm: TIRE_CHECK_INTERVAL, ultimaKm: 0, status: "OK" }, revisaoGeral: { proximaKm: GENERAL_SERVICE_INTERVAL, ultimaKm: 0, status: "OK" } };
    ...
    frear(decremento = 10) { if (this.velocidade > 0) { this.velocidade = Math.max(0, this.velocidade - decremento); this.atualizarDisplayCompleto(); } }
    buzinar() { showNotification(`${this.modelo}: Beep beep!`, 'info'); }
    adicionarRegistroHistorico(tipo, obs) { this.historicoManutencao.push({ data: new Date().toISOString(), tipoServico: tipo, kmNaManutencao: this.kmRodados.toFixed(0), observacoes: obs }); }
    verificarManutencao() { Object.entries({ oleo: OIL_CHANGE_INTERVAL, pneus: TIRE_CHECK_INTERVAL, revisaoGeral: GENERAL_SERVICE_INTERVAL }).forEach(([key, interval]) => { const item = this.manutencao[key]; if (this.kmRodados >= item.proximaKm) item.status = "VENCIDO"; else if (this.kmRodados >= item.proximaKm - interval / 5) item.status = "PRÓXIMO"; else item.status = "OK"; }); }
    realizarManutencaoItem(key, interval, name) { const item = this.manutencao[key]; item.ultimaKm = this.kmRodados; item.proximaKm = this.kmRodados + interval; this.adicionarRegistroHistorico(name, `Realizado em ${this.kmRodados.toFixed(0)} km`); this.verificarManutencao(); this.atualizarDisplayCompleto(); showNotification(`${name} para ${this.modelo} realizada.`, 'success'); }
    realizarTrocaOleo() { this.realizarManutencaoItem('oleo', OIL_CHANGE_INTERVAL, 'Troca de Óleo'); }
    verificarPneus() { this.realizarManutencaoItem('pneus', TIRE_CHECK_INTERVAL, 'Verificação de Pneus'); }
    realizarRevisaoGeral() { this.realizarManutencaoItem('revisaoGeral', GENERAL_SERVICE_INTERVAL, 'Revisão Geral'); this.adicionarRegistroHistorico("Revisão Geral", "Todos os itens verificados."); this.verificarManutencao(); this.atualizarDisplayCompleto(); }
    ...
    atualizarDisplayCompleto() {
        if (garagem.veiculoSelecionado?.id !== this.id) return;
        const imgElement = document.getElementById('vehicle-image');
        imgElement.src = this.imagemURL;
        // imgElement.onerror = () => { imgElement.onerror = null; imgElement.src = FALLBACK_IMAGE_URL; };
        imgElement.alt = `Imagem de ${this.modelo}`;
        ...
        document.getElementById('maintenance-status-report').innerHTML = Object.entries(this.manutencao).map(([key, val]) => `<strong>${key.charAt(0).toUpperCase() + key.slice(1).replace("Geral", " Geral")}:</strong> ${formatStatus(val.status)} (Próx.: ${val.proximaKm.toFixed(0)} km)`).join('<br>');
...
class Garagem {
    ...
    removerVeiculo(idVeiculo) { const idNum = parseInt(idVeiculo, 10); const index = this.veiculos.findIndex(v => v.id === idNum); if (index > -1) { const removido = this.veiculos.splice(index, 1)[0]; showNotification(`${removido.constructor.name.replace(/([A-Z])/g, ' $1').trim()} ${removido.modelo} removido.`, 'success'); if (this.veiculoSelecionado?.id === idNum) { this.veiculoSelecionado = null; document.getElementById('selected-vehicle-panel').style.display = 'none'; document.getElementById('vehicle-selector').value = ""; } this.agendamentos = this.agendamentos.filter(ag => ag.idVeiculo !== idNum); this.atualizarSeletorVeiculos(); updateAppointmentVehicleSelector(); renderAppointmentsTable(); this.saveToLocalStorage(); updateGarageStatus(); } }
...
function switchView(viewId) { viewSections.forEach(s => s.classList.toggle('active', s.id === viewId)); sidebarLinks.forEach(l => { l.classList.toggle('active-view', l.dataset.view === viewId); if (l.dataset.view === viewId) viewTitleElement.textContent = l.textContent.replace(/<i.*><\/i>\s*/, ''); }); if (viewId === 'scheduling-view') { updateAppointmentVehicleSelector(); renderAppointmentsTable(); } if (viewId === 'garage-view' && garagem.veiculoSelecionado) { switchVehicleDetailTab('details'); } if (viewId === 'arsenal-view') { loadArsenalData(); } }
function switchVehicleDetailTab(tabId) { vehicleTabContents.forEach(c => c.classList.toggle('active', c.id === `vehicle-${tabId}-tab`)); vehicleTabButtons.forEach(b => b.classList.toggle('active', b.dataset.tab === tabId)); }
function renderMaintenanceHistory(veiculo) { const tableBody = document.getElementById('maintenance-history-table').querySelector('tbody'); const noHistoryMsg = document.getElementById('no-maintenance-history'); document.getElementById('history-vehicle-model').textContent = veiculo.modelo; tableBody.innerHTML = ''; if (veiculo.historicoManutencao?.length) { noHistoryMsg.style.display = 'none'; veiculo.historicoManutencao.slice().reverse().forEach(item => { const r = tableBody.insertRow(); r.insertCell().textContent = new Date(item.data).toLocaleDateString('pt-BR'); r.insertCell().textContent = item.tipoServico; r.insertCell().textContent = item.kmNaManutencao; r.insertCell().textContent = item.observacoes || '-'; }); } else noHistoryMsg.style.display = 'block'; }
function updateAppointmentVehicleSelector() { const selector = document.getElementById('appointment-vehicle-selector'); const currentVal = selector.value; selector.innerHTML = '<option value="">-- Selecione um Veículo --</option>'; garagem.veiculos.forEach(v => { const opt = document.createElement('option'); opt.value = v.id; opt.textContent = `${v.modelo} (${v.cor})`; selector.appendChild(opt); }); if (garagem.veiculos.find(v => v.id.toString() === currentVal)) selector.value = currentVal; }
function renderAppointmentsTable() { const tableBody = document.getElementById('appointments-table').querySelector('tbody'); const noAppointmentsMsg = document.getElementById('no-appointments'); tableBody.innerHTML = ''; if (garagem.agendamentos.length) { noAppointmentsMsg.style.display = 'none'; garagem.agendamentos.forEach(ag => { const r = tableBody.insertRow(); r.insertCell().textContent = ag.dataFormatada; r.insertCell().textContent = ag.nomeVeiculo; r.insertCell().textContent = ag.tipoServico; r.insertCell().innerHTML = `<span class="status-${ag.status.toLowerCase()}">${ag.status}</span>`; const actionsCell = r.insertCell(); if (ag.status === "Agendado") { ['Concluir', 'Cancelar'].forEach(action => { const btn = document.createElement('button'); btn.textContent = action; btn.classList.add('action-button', action.toLowerCase()); btn.onclick = () => garagem.atualizarStatusAgendamento(ag.id, action === 'Concluir' ? 'Concluído' : 'Cancelado'); actionsCell.appendChild(btn); }); } else { const btn = document.createElement('button'); btn.textContent = "Remover"; btn.classList.add('action-button', 'remove'); btn.onclick = () => { if (confirm(`Remover agendamento ${ag.status.toLowerCase()}?`)) garagem.removerAgendamento(ag.id); }; actionsCell.appendChild(btn); } }); } else noAppointmentsMsg.style.display = 'block'; }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('add-vehicle-button').addEventListener('click', () => {
        ...
        try { let novoVeiculo; if (type === 'Caminhao') { const cap = parseInt(document.getElementById('new-truck-capacity').value, 10); if (isNaN(cap) || cap <= 0) { showNotification("Capacidade de carga inválida.", 'error'); return; } novoVeiculo = new Caminhao(modelo, cor, cap, img); } else if (type === 'CarroEsportivo') { novoVeiculo = new CarroEsportivo(modelo, cor, img); } else { novoVeiculo = new Veiculo(modelo, cor, img); } garagem.adicionarVeiculo(novoVeiculo); ['new-vehicle-model', 'new-vehicle-color', 'new-vehicle-image', 'new-truck-capacity'].forEach(id => document.getElementById(id).value = ''); document.getElementById('vehicle-type-selector').value = "Veiculo"; document.getElementById('new-truck-capacity').style.display = 'none'; } catch (err) { showNotification(`Erro: ${err.message}`, 'error'); }
    });
    document.getElementById('vehicle-selector').addEventListener('change', e => garagem.selecionarVeiculo(e.target.value));
    document.getElementById('remove-vehicle-button').addEventListener('click', () => { if (garagem.veiculoSelecionado && confirm(`Remover ${garagem.veiculoSelecionado.modelo}?`)) { garagem.removerVeiculo(garagem.veiculoSelecionado.id); } });
    document.getElementById('selected-vehicle-panel').addEventListener('click', e => { if (e.target.tagName === 'BUTTON' && e.target.dataset.action && garagem.veiculoSelecionado) { const acao = e.target.dataset.action; const v = garagem.veiculoSelecionado; try { if (typeof v[acao] === 'function') { v[acao](); } else if (acao === 'carregar' || acao === 'descarregar') { const peso = document.getElementById('truck-cargo-weight').value; v[acao](peso); document.getElementById('truck-cargo-weight').value = ''; } else if (acao.startsWith('service')) { const serviceMap = { serviceOil: 'realizarTrocaOleo', serviceTires: 'verificarPneus', serviceGeneral: 'realizarRevisaoGeral' }; if (serviceMap[acao]) v[serviceMap[acao]](); } } catch (err) { showNotification(`Erro: ${err.message}`, 'error'); } } });
    document.getElementById('add-appointment-button').addEventListener('click', () => {
        ...
        if (veiculo) { try { garagem.adicionarAgendamento(new Agendamento(vehId, veiculo.modelo, date, type, notes)); ['appointment-vehicle-selector', 'appointment-date', 'appointment-service-type', 'appointment-notes'].forEach(id => document.getElementById(id).value = ''); } catch (err) { showNotification(`Erro: ${err.message}`, 'error'); } }
    });
